...
function priceTicketsFromComp(comp, qty) {
  ...
  const tier = comp.ticketTiers.find(t => Number(t.amount) === Number(qty));
  if (!tier) throw new HttpsError("invalid-argument", "Invalid ticket bundle.");
  const totalPence = Math.round(Number(tier.price) * 100);
  return { totalPence, tierPrice: Number(tier.price) };
}
...
exports.allocateTicketsAndAwardTokens = onCall(functionOptions, async (request) => {
  ...
  return await db.runTransaction(async (transaction) => {
    ...
    const tier = compData.ticketTiers.find(t => Number(t.amount) === Number(ticketsBought));
    if (!tier) throw new HttpsError('invalid-argument', 'Selected ticket bundle is not valid for this competition.');

    // Compute server-side price
    const priceToCharge = Number(tier.price);
